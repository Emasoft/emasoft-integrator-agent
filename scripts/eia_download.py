#!/usr/bin/env python3
"""
EIA Document Download and Storage Manager

Downloads .md files from GitHub issue comments and stores them in the
standardized EIA folder structure with read-only enforcement.

Usage:
    python eia_download.py download --url URL --task-id TASK_ID --category CATEGORY
    python eia_download.py init --project-root PATH
    python eia_download.py lookup --task-id TASK_ID
    python eia_download.py verify --project-root PATH
"""
...
def get_storage_root(project_root: Path | None = None) -> Path:
    """Get the EIA storage root directory."""
    if project_root:
...
def init_storage(project_root: Path) -> None:
    """Initialize the EIA storage directory structure."""
    storage_root = get_storage_root(project_root)

    print(f"Initializing EIA storage at: {storage_root}")
    ...
    gitkeep.write_text("# EIA document storage - do not delete this folder\n")

    # Update .gitignore if in git repo
    gitignore_path = project_root / ".gitignore"
    gitignore_entry = "\n# EIA Document Storage (local cache)\n.eia/\n!.eia/.gitkeep\n"
    ...
    print("EIA storage initialized successfully")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="EIA Document Download and Storage Manager",
    ...
    if args.command == "init":
        ...
        if args.json:
            ...
            print("\n=== EIA Storage Verification Report ===\n")

...
**Using eia_get_pr_files.py:**

```bash
# Get all changed files with their extensions
python3 eia_get_pr_files.py --pr 123 | jq -r '
...
**Using eia_get_pr_diff.py with --stat:**
...
**Using eia_get_pr_diff.py with --files:**

```bash
# Get diff for specific files only
python3 eia_get_pr_diff.py --pr 123 --files src/auth.py src/models/user.py

# Get diff for files matching a pattern (use shell expansion or jq filtering)
python3 eia_get_pr_files.py --pr 123 | jq -r '
  .[] | select(.filename | test("^src/")) | .filename' |
  xargs python3 eia_get_pr_diff.py --pr 123 --files
```
...
FILES=$(python3 eia_get_pr_files.py --pr 123 | jq -r '
  [.[] | select(.filename | startswith("src/"))] | .[].filename')
python3 eia_get_pr_diff.py --pr 123 --files $FILES
```

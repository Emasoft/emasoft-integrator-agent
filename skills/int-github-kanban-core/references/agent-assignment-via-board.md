...
**The GitHub issue assignee IS the assigned agent. There is no other assignment mechanism.**

This means:
- To assign work to Agent A, set issue assignee to Agent A
- To check who owns work, look at issue assignee
- To reassign work, change the assignee
- If no assignee, the work is unassigned

### Why Assignees as Source of Truth?
...
| Custom labels | Clutters label space |

### Assignment Rules

1. **One primary assignee per module** - The first assignee is the owner
2. **Assignment moves with status** - Assignee persists across status changes
3. **Unassigned = Available** - Items without assignee can be picked up

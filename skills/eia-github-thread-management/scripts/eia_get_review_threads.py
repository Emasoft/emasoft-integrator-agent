...
Usage:
    python3 eia_get_review_threads.py --owner OWNER --repo REPO --pr NUMBER [--unresolved-only]

Output:
    JSON array of thread objects with id, isResolved, path, line, body (first comment).

Example:
    python3 eia_get_review_threads.py --owner octocat --repo Hello-World --pr 42
    python3 eia_get_review_threads.py --owner octocat --repo Hello-World --pr 42 --unresolved-only
